<?php
/**
 * Template part for displaying posts in a Justified Grid (loop01)
 *
 * Copies Stories content-* card design inside the justified grid wrapper.
 *
 * @package Stories
 * @since 1.0.0
 */
...
<div class="ajax-item-wrapper" 
	...
	<?php
	// Render format-specific Stories content card for loop01
	if ( 'gallery' === $format && locate_template( 'template-parts/loop01/content-gallery.php' ) ) {
		get_template_part( 'template-parts/loop01/content', 'gallery' );
	} elseif ( 'image' === $format && locate_template( 'template-parts/loop01/content-image.php' ) ) {
		get_template_part( 'template-parts/loop01/content', 'image' );
	} elseif ( locate_template( "template-parts/loop01/content-{$format}.php" ) ) {
		get_template_part( 'template-parts/loop01/content', $format );
	} elseif ( locate_template( 'template-parts/loop01/content.php' ) ) {
		get_template_part( 'template-parts/loop01/content' );
	} else {
