 ...
 * If no gallery block or shortcode is found, it collects images from the post
 * and converts them into a gallery.
 *
 * @package Stories
 * @since 1.0.5
 */
...
if ( ! function_exists( 'stories_collect_image_ids_from_blocks' ) ) {
	...
	function stories_collect_image_ids_from_blocks( $blocks ) {
		...
		foreach ( $blocks as $block ) {
				...
				if ( ! empty( $block['attrs']['ids'] ) && is_array( $block['attrs']['ids'] ) ) {
					...
					if ( ! empty( $block['innerBlocks'] ) ) {
						foreach ( $block['innerBlocks'] as $ib ) {
							if ( ! empty( $ib['blockName'] ) && 'core/image' === $ib['blockName'] ) {
			...
			// --- 2) core/image simple (standalone image blocks in the post) ---
			...
			// --- 3) General recursion into innerBlocks (e.g. columns, groups, etc.) ---
			if ( ! empty( $block['innerBlocks'] ) ) {
...
if ( ! function_exists( 'stories_extract_gallery_images' ) ) {
	 ...
	 * If no gallery block or shortcode is found, it collects all images from the post
	 * and returns their IDs to form a gallery.
	 *
	...
	function stories_extract_gallery_images( $post_id ) {
		...
		// 1. Try parsing Gutenberg blocks (both core/gallery and standalone core/image blocks)
		if ( function_exists( 'parse_blocks' ) && ! empty( $content ) ) {
			$blocks    = parse_blocks( $content );
			$image_ids = stories_collect_image_ids_from_blocks( $blocks );
		}

		// 2. Try classic shortcode [gallery ids="..."]
		...
		// 4. Try extracting <img> tags from content if no blocks were parsed
		if ( empty( $image_ids ) && ! empty( $content ) ) {
		...
		// 5. Check attached media (attachments with post_parent = $post_id)
		if ( empty( $image_ids ) ) {
...
if ( ! function_exists( 'avante_extract_gallery_images' ) ) {
	/**
	 * Compatibility alias for templates or snippets borrowing Avante's function name.
	 *
	 * @param int $post_id The post ID.
	 * @return array Array of image IDs.
	 */
	function avante_extract_gallery_images( $post_id ) {
		return stories_extract_gallery_images( $post_id );
	}
}
